...
/**
 * Downloads a complete, branded VIP Loyalty Pass Card image as a high-resolution PNG
 * (Clean template without static punch-card circles, preserving high-contrast scannable QR code)
 */
...
/**
 * Downloads just the clean standalone QR code PNG image
 */
export async function downloadBathaqueQrCodeOnly(bathaqueId, customerName = 'Customer') {
  if (!bathaqueId) throw new Error('Bathaque ID is required');
  ...
  const cleanName = customerName.replace(/[^a-zA-Z0-9_-]/g, '_');
  const link = document.createElement('a');
  link.download = `QR_${bathaqueId}_${cleanName}.png`;
  link.href = qrDataUrl;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
