...
conn.on('ready', async () => {
  ...
  try {
    ...
    const backendRes = await executeCommand(conn, `cd ${repoPath}/backend && npm install && node scripts/fix-suppliers-table.js && node scripts/fix-existing-stamps.js && node scripts/fix-free-services-prices.js && pm2 restart all`);
