...
async function run() {
  try {
    ...
    const [services] = await conn.query(
      "SELECT id, name, category, price, vehicle_type FROM products WHERE category = 'Services' ORDER BY name ASC"
    );
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    const writeStream = sftp.createWriteStream('/tmp/inspect_services.js');
    writeStream.on('close', () => {
      conn.exec('node /tmp/inspect_services.js', (err, stream) => {
        if (err) { conn.end(); return; }
