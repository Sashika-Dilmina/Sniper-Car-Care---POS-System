...
async function run() {
  try {
    ...
    const [rows] = await conn.query("SELECT id, status, opened_at, closed_at FROM cash_registers ORDER BY id DESC LIMIT 5");
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    const stream = sftp.createWriteStream('/tmp/inspect_vps_registers.js');
    stream.on('close', () => {
      conn.exec('cd /root/Sniper-Car-Care---POS-System/backend && node /tmp/inspect_vps_registers.js && rm -f /tmp/inspect_vps_registers.js', (err, stream) => {
        if (err) {
          console.error(err);
          conn.end();
          return;
