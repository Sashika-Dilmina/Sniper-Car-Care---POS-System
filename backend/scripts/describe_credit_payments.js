...
async function run() {
  try {
    ...
    const [columns] = await conn.query("DESCRIBE credit_payments");
    console.log(columns);
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    const writeStream = sftp.createWriteStream('/tmp/describe_credit_payments.js');
    writeStream.on('close', () => {
      conn.exec('node /tmp/describe_credit_payments.js', (err, stream) => {
        if (err) { conn.end(); return; }
