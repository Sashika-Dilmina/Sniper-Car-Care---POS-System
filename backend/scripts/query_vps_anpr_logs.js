...
async function run() {
  try {
    ...
    const [logs] = await conn.query(
      "SHOW CREATE TABLE anpr_logs"
    );
    console.log('--- ANPR_LOGS TABLE SCHEMA ---');
    console.log(logs[0]['Create Table']);
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    const writeStream = sftp.createWriteStream('/tmp/query_vps_anpr_logs.js');
    writeStream.on('close', () => {
      conn.exec('node /tmp/query_vps_anpr_logs.js', (err, stream) => {
        if (err) { conn.end(); return; }
