...
async function run() {
  try {
    ...
    const [rows] = await conn.query("SHOW CREATE TABLE credit_payments");
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    const writeStream = sftp.createWriteStream('/tmp/show_create_payments.js');
    writeStream.on('close', () => {
      conn.exec('node /tmp/show_create_payments.js', (err, stream) => {
        if (err) { conn.end(); return; }
