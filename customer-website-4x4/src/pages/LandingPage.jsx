...
const LandingPage = () => {
  ...
  useEffect(() => {
    if (bookingSuccessData) {
      axios.get('/api/public/extra-services?vehicle_type=4x4')
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await axios.get('/api/public/products?vehicle_type=4x4');
  ...
  const handleServiceClick = (service) => {
    ...
    if (customerInfo) {
      ...
      submitBooking(service, {
        name: customerInfo.name,
        phone: customerInfo.phone || '+9715',
        vehicle_type: customerInfo.vehicle_type || '4x4',
